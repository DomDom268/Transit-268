...
your_api_key = os.getenv('VEHICLE_6_KEY') #API key for authentication, generated when vehicle is added to database. Can be found in the vehicles table in the database
...
while True:
    ...
    lat,lon = route[i]
    route_id = 14 if direction == 1 else 13
    payload = {
        'vehicle_id' : 6,
        'vehicle_plate' : 'TEST128',
        'vehicle_name' : 'Pumpy',
        # 'vehicle_type' : 'Bus',
        'route_id': route_id,
        'latitude' : lat,
